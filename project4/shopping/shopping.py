...
def main():
    ...
    # Load data from spreadsheet and split into train and test sets  'C:\\Users\\User\\Desktop\\PythonFiles\\shopping\\shopping.csv'
...
def load_data(filename):
    ...
    with open(filename) as f:
        for line in csv.DictReader(f):
            evidence.append([
                int(line['Administrative']), float(line['Administrative_Duration']), 
                ...
                float(line['SpecialDay']), Months[line['Month']], int(line['OperatingSystems']), 
                int(line['Browser']), int(line['Region']), int(line['TrafficType']), 
                0 if line['VisitorType'] == 'New_Visitor' else 1, 
                0 if line['Weekend'] == 'FALSE' else 1
            ])
...
def evaluate(labels, predictions):
    ...
    """
    sensitivity = specificity = positives = negatives = 0.0
